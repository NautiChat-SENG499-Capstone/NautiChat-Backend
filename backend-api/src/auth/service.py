...
async def get_user_by_token(
    token: str, settings: config.Settings, db: AsyncSession
) -> UserModel:
...
async def register_user(
    user_register: CreateUserRequest, settings: config.Settings, db: AsyncSession
) -> Token:
...
async def login_user(
    form_data: OAuth2PasswordRequestForm, settings: config.Settings, db: AsyncSession
) -> Token:
