...
def vdb_auto_upload(app_state):
    try:
        ...
        print(f"{get_current_time()} vector DB auto upload - Deleting {filtered_points.count} points from {qdrant_client_wrapper.general_collection_name}")
        qdrant_client_wrapper.qdrant_client.delete( 
            qdrant_client_wrapper.general_collection_name, points_selector=filter_condition
        )
        ...
        prepare_embedding_input = prepare_embedding_input_from_preformatted(input=inputs, embedding_model=JinaEmbeddings(), doChunking=False)

        print(f"{get_current_time()} vector DB auto upload - Uploading {len(prepare_embedding_input)} points to {qdrant_client_wrapper.general_collection_name}")
